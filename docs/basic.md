...
- 오르막수 : Dij = D[i-1][0] +..+ D[i-1][j]

### D가 다차원 배열인 경우

- 쉬운 계단수 : D[계단수의 길이][계단수의 마지막 숫자] | 오르막수 : D[오르막수 길이][오르막수 마지막 숫자]

### D가 다차원 배열에서 한 차원 줄인 경우

- 포도주 시식 : d[n번째까지먹음][연속으로 마신횟수] -> d[n] : (마음속 case 0번 연속) d[n-1] (마음속 case 1번연속) d[n-2] + p[i](3번연속) d[n-2] + p[i-1] + p[i] -> max
- 이친수 : 마음속으로 case를 만들어 : d[n]이 0으로 끝나는case: d[n-1] | d[n]이 1로끝나는 case : d[n-2] (d[n-1]은 반드시 0이여야됨 d[n-2]부터 뭐가와든 상관없어.) => 피보나치
